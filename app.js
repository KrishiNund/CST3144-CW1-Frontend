...
let app = new Vue({
  ...
  methods: {
    ...
    removeItemFromCart(lesson) {
      let newLessonSpace = lesson.spaces++;
      let indexToBeRemoved = this.cartArray.findIndex(
        (item) =>
          item.subject === lesson.subject && item.spaces === lesson.spaces
      );
    ...
    updateLesson() {
      ...
      for (let lesson of lessonSet) {
        ...
        fetch(
          "https://cst3144-m00934333-cw1-backend.onrender.com/api/updateLesson",
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
    ...
    getFilteredLessons() {
      ...
      fetch(
        `https://cst3144-m00934333-cw1-backend.onrender.com/api/search?query=${encodedQuery}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
